...
var metaRow = metaCells.map(function (c) {
  return '<td><span class="mk">' + c[0] + ':</span> ' + c[1] + "</td>";
}).join("");
...
const html = `<!doctype html><html><head><meta charset="utf-8"><style>
  ...
  * { font-family: "Noto Serif", "Times New Roman", serif; }
  ...
  table.metabar td { border: 0.75px solid #000; padding: 4px 8px; }
  table.metabar .mk { color: #444; }
  ...
  .ginstr ol { margin: 0; padding-left: 18px; }
  .ginstr li { margin: 1px 0; }
  .flow { ${twoCol ? "column-count: 2; column-gap: 8mm; column-rule: 0.5px solid #ccc;" : ""} }
  h2.sec { font-size: 12.5pt; text-align: center; margin: 12px 0 2px; border-top: 1px solid #000;
           padding-top: 5px; ${twoCol ? "column-span: all;" : ""} }
  .secnote { text-align: center; font-size: 9.5pt; color: #444; font-style: italic; margin: 0 0 7px;
             ${twoCol ? "column-span: all;" : ""} }
  .q { margin: 0 0 9px; break-inside: avoid; }
  ...
  .case { background: #f4f4f4; border: 0.5px solid #999; padding: 6px 8px; margin: 6px 0; font-size: 10.3pt;
          break-inside: avoid; ${twoCol ? "column-span: all;" : ""} }
  .katex { font-size: 1.02em; }
</style></head><body>
  <div class="head">
    <h1>${esc(data.paper_title || "Question Paper")}</h1>
    ${data.book_title ? `<div class="sub">${esc(data.book_title)}</div>` : ""}
  </div>
  <table class="metabar"><tr>${metaRow}</tr></table>
  ${instructionsHtml()}
